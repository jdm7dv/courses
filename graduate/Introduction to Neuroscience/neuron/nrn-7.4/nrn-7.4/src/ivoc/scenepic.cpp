...
MenuItem* K::menu_item(const char* name) {
	Glyph* g = LayoutKit::instance()->r_margin(
...
ButtonItemInfo::ButtonItemInfo(const char* name, Action* a, TelltaleState* s, MenuItem* mi, Menu* parent)
{
	name_ = name;
	s_ = s;
	a_ = a;
...
/*static*/ class RoundView : public Action {
	virtual void execute();
};

/*static*/ class WholeSceneView : public Action {
	virtual void execute();
};

/*static*/ class WholePlotView : public Action {
	virtual void execute();
};

/*static*/ class SpecView : public Action {
	virtual void execute();
};

/*static*/ class SizeScene : public Action {
	virtual void execute();
};

/*static*/ class ZoomOut10: public Action {
	virtual void execute();
};

/*static*/ class ZoomIn10: public Action {
	virtual void execute();
};

/*static*/ class SPObjectName: public Action {
	virtual void execute();
};
...
TelltaleGroup* ScenePicker::telltale_group(){return spi_->tg_;}
...
Button* ScenePicker::radio_button(const char* name, Action* a) {
	Button* mi = WidgetKit::instance()->radio_button(spi_->tg_, name,
		new RadioSelect(name, a, spi_->scene_));
	spi_->bil_->append(new ButtonItemInfo(name, mi->action(), mi->state()));
	return mi;
}
MenuItem* ScenePicker::add_radio_menu(const char* name, Rubberband* rb, Action* sel, int tool, Menu* m) {
...
void ScenePicker::insert_item(const char* insert, const char* name, MenuItem* mi) {
	...
	if (i > -1) {
		...
		if (j > -1) {
			...
			spi_->bil_->insert(i, new ButtonItemInfo(name,
				mi->action(), mi->state(), mi, b->parent_));
		}
	}
}
...
ScenePickerImpl::ScenePickerImpl(Scene* scene) : sel_name_(""){
	menu_ = new PopupMenu();
	menu_->ref();
...
RadioSelect::RadioSelect(const char* name, Action* a, Scene* s) : name_(name) {
	a_ = a;
	Resource::ref(a_);
	s_ = s; // not referenced
}
...
void NewView::execute(Rubberband* rb) {
	...
	ViewWindow* w = new ViewWindow(v,
		((PrintableWindow*)(cv->canvas()->window()))->type());
