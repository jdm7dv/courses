...
void Axis::save(ostream& o) {
	...
	sprintf(buf, "save_window_.%caxis(%g,%g,%g,%d,%d,%d,%d)",
		c, amin_, amax_, pos_, ntic_, nminor_, invert_, number_);
	o << buf << endl;
}
...
void Axis::install() {
	...
	if (invert_ == 1) { l_minor *= -1.; length *= -1.; }
	...
	while (s < 1) s *= 10.;
	if (s == 1 || s == 2) addprec = 0; else addprec = 1;
	...
	if (d_ == Dimension_X) {
	  ...
	  if (logstep >= 0 && logstep <= 5) {
		...
		for (i=0; i <= ntic_; ++i) {
			...
			*/
			  x_align = 0.5;
			  if (invert_ == 1) y_align = -0.3; else y_align = 1.5;
			  sprintf(str, pform, x);
			  s_->append_fixed(new GAxisItem(
			   new GLabel(str, Appear::default_color(),
				      true, 1, x_align, y_align)));
			  gi = s_->count() - 1;
			  s_->move(gi, x, y);
			}
	  ...
	  if (logstep >= 0 && logstep <= 5) {
		...
		for (i=0; i <= ntic_; ++i) {
			...
			if (number_) {
			  ...
			  //else y_align = 0.33;
			  if (invert_ == 1) x_align = 0; else x_align = 1.3;
			  s_->append_fixed(new GAxisItem(
	 		    new GLabel(str, Appear::default_color(),
				       true, 1, x_align, y_align)));
			  gi = s_->count() - 1;
...
void BoxBackground::tic_label( Coord x1, Coord y1, Coord val,
	float xa, float ya, Canvas* c) const
{
	char buf[20];
	sprintf(buf, "%g", val);
...
void AxisBackground::tic_label( Coord x1, Coord y1, Coord val,
	float xa, float ya, Canvas* c) const
{
	char buf[20];
	sprintf(buf, "%g", val);
