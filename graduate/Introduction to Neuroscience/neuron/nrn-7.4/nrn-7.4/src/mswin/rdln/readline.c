...
{
#ifdef WIN32
	winio_size(&screenheight, &screenwidth);
#else
#if defined(__GO32__)
...
{
#ifdef WIN32
	winio_output_some_chars(string, count);
#else
#if GRX
 if (egagrph) {
...
{
#ifdef WIN32
	winio_delete_chars(count);
#else
#if defined(__GO32__)
  int r, c, w;
#if GRX
 if (egagrph) {
	grx_delete_chars(count);
 }else
#endif
 {
  ScreenGetCursor(&r, &c);
...
{
#ifdef WIN32
	winio_insert_some_chars(string, count);
#else
#if defined(__GO32__)
  int r, c, w;
#if GRX
 if (egagrph) {
	grx_insert_some_chars(string, count);
 }else
#endif
 {
  ScreenGetCursor(&r, &c);
...
rl_deprep_terminal ()
{
#if !defined(__GO32s__) && !defined(WIN32)
  int tty = fileno (rl_instream);
  SIGNALS_DECLARE_SAVED (saved_signals);
...
{
#ifdef WIN32
printf("implement rl_refresh_line\n");
#else
...
{
#ifdef WIN32
printf("implement rl_clear_screen\n");
#else
